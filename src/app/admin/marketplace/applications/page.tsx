...
export default function AdminApplicationsPage() {
  ...
  async function loadAgencies() {
    try {
      const res = await apiCall('/admin/agencies/options');
        ...
        <div className="p-6 max-w-5xl mx-auto">
          ...
          <div className="finance-card p-4 mb-4">
            <div className="space-y-3">
              {/* First row: Status, Agency, Search */}
              <div className="flex flex-col md:flex-row gap-3">
                ...
                <div className="flex-1">
                  <label className="text-sm" style={{ color: '#535353' }}>{t('page.text.agency')}</label>
                  <select
                    className="w-full mt-1 bg-white/50 border rounded p-2"
                    value={agencyId}
                    onChange={(e) => setAgencyId(e.target.value)}
                  >
                    <option value="">{t('page.text.allagencies')}</option>
                    {agencies.map((c) => (
                      <option key={c.id} value={c.id}>{c.agency_name}</option>
                    ))}
                  </select>
                </div>
                <div className="flex-1">
              ...
              <div className="flex flex-col md:flex-row md:items-end gap-3">
                ...
                <div className="flex gap-2">
                  ...
                  <Button variant="outline" className="bg-white/60 border-white/50" onClick={() => { setStatus(''); setAgencyId(''); setSearch(''); setDateFrom(''); setDateTo(''); setPage(1); load(); }}>
