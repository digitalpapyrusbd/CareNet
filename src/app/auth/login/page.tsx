    ...
    try {
      ...
      if (response.requiresMFA) {
        ...
        if (response.tempSessionId) {
          localStorage.setItem("mfaTempSessionId", response.tempSessionId);
        }
      ...
      // Normal login response - store tokens (backend returns accessToken and refreshToken in camelCase)
      ...
      if (error.status === 0) {
        ...
        if (Array.isArray(validationErrors)) {
          ...
          errorMessage =
            error.data?.message ||
            "Invalid input. Please check your phone number and password format.";
        }
