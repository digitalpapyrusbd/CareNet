import { Award, CheckCircle, XCircle, AlertCircle, FileText } from "lucide-react";
import { Button } from "../ui/button";
import { Textarea } from "../ui/textarea";
import { useState } from "react";

interface CaregiverCertificateReviewProps {
  submissionId: string;
...
export function CaregiverCertificateReview({
  submissionId,
  caregiverName,
  moderatorName,
  moderatorDecision,
...
}: CaregiverCertificateReviewProps) {
  ...
  return (
    <div className="min-h-screen p-6">
      <div className="max-w-4xl mx-auto">
        <div className="mb-6">
          <h1 style={{ color: '#535353' }}>Certificate Review</h1>
        ...
        <div className="finance-card p-6 mb-6">
          ...
          <div className="space-y-2">
            ...
            <div className="flex justify-between">
              <span style={{ color: '#848484' }}>Total Certificates:</span>
              <span style={{ color: '#535353' }}>{certificates.length}</span>
            </div>
          </div>
        </div>
        ...
        {/* Certificates */}
        <div className="finance-card p-6 mb-6">
          <h3 className="mb-4" style={{ color: '#535353' }}>Submitted Certificates</h3>
          <div className="space-y-3">
            {certificates.map((cert, index) => (
              <div key={index} className="p-4 rounded-lg" style={{ background: 'rgba(255, 255, 255, 0.5)' }}>
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-start gap-3">
                    <Award className="w-5 h-5 shrink-0" style={{ color: '#7CE577' }} />
                    <div>
                      <h4 className="mb-1" style={{ color: '#535353' }}>{cert.name}</h4>
                      <p className="text-sm" style={{ color: '#848484' }}>Issued by: {cert.issuer}</p>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-3 text-sm">
                  <div>
                    <span style={{ color: '#848484' }}>Issue Date: </span>
                    <span style={{ color: '#535353' }}>{cert.dateIssued}</span>
                  </div>
                  {cert.expiryDate && (
                    <div>
                      <span style={{ color: '#848484' }}>Expiry: </span>
                      <span style={{ color: '#535353' }}>{cert.expiryDate}</span>
                    </div>
                  )}
                </div>
              </div>
