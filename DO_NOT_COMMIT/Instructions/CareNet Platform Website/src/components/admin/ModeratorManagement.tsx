...
export function ModeratorManagement({ moderators, onAddModerator, onViewDetails, onEdit, onSuspend }: ModeratorManagementProps) {
    ...
    <div className="min-h-screen pb-6">
      <div className="p-6">
        <div className="flex items-center justify-between mb-6">
          ...
          <Button onClick={onAddModerator} size="sm"
              ...
              return (
                <div key={moderator.id} className="finance-card p-4">
                  ...
                  <div className="flex gap-2">
                    <Button onClick={() => onViewDetails(moderator.id)} size="sm" variant="outline"
                      className="flex-1 bg-white/50 border-white/50">
                      <Eye className="w-4 h-4 mr-2" />View
                    </Button>
                    <Button onClick={() => onEdit(moderator.id)} size="sm" variant="outline"
                      className="flex-1 bg-white/50 border-white/50">
                      <Edit className="w-4 h-4 mr-2" />Edit
                    </Button>
                    {moderator.status === 'active' && (
                      <Button onClick={() => onSuspend(moderator.id)} size="sm" variant="outline"
