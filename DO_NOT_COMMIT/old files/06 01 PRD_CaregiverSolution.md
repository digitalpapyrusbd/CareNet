...
**Backend**: NestJS (Node.js) REST API (deployed on Render)
**Database**: Prisma ORM with PostgreSQL (Neon - production)
**Cache**: Redis (Upstash) for sessions, rate limiting, OTP codes
**File Storage**: Cloudflare R2 (S3-compatible)
**Authentication**: JWT with refresh tokens, MFA for sensitive roles (SUPER_ADMIN, MODERATOR, COMPANY)
**Payment Gateways**: bKash, Nagad (with escrow system)
