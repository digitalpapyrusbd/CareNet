...
## Error State Diagrams (Payment, Auth, Webhooks)

These diagrams show error handling branches and recovery flows for critical integrations: payments, authentication, and webhooks.

### Payment Flow — Error States
```mermaid
flowchart TD
  Payment[Start Payment] --> RequestToken[Request Payment Token]
  RequestToken --> |200 OK| Checkout[Open Checkout]
  RequestToken --> |401| TokenErr[Unauthorized - Invalid Credentials]
  TokenErr --> RetryToken{Retry?}
  RetryToken --> |Yes| RequestToken
  RetryToken --> |No| ContactSupport[Contact Support]

  Checkout --> |User Completes| Confirm[Confirm Payment]
  Checkout --> |User Cancels| Cancelled[Payment Cancelled]

  Confirm --> |Success| PaymentSuccess[Payment Success]
  Confirm --> |Failed| PaymentFailed[Payment Failed]

  PaymentFailed --> RetryPayment{Retry Payment?}
  RetryPayment --> |Yes| Checkout
  RetryPayment --> |No| ManualResolve[Manual Resolution]
  ManualResolve --> |Refund| RefundProcess[Process Refund]
  ManualResolve --> |Escalate| ContactSupport

  PaymentSuccess --> JobTracking[Job Tracking Dashboard]
  RefundProcess --> GuardianNotified[Notify Guardian] --> End
  ContactSupport --> End
```
...
```mermaid
flowchart TD
  Webhook[Receive Webhook] --> ValidateSig[Validate Signature]
  ValidateSig --> |Valid| ProcessEvent[Process Event]
  ValidateSig --> |Invalid| DropAndLog[Drop + Log]

  ProcessEvent --> |Idempotent check passes| Ack[Send 200 Ack]
  ProcessEvent --> |Duplicate| SkipProcessing[Skip duplicate] --> Ack

  ProcessEvent --> |Processing error| ScheduleRetry[Schedule Retry (1m,5m,30m,...)]
  ScheduleRetry --> RetryAttempt[Retry Attempt]
  RetryAttempt --> |Still failing| Escalate[Escalate to admin]
  Escalate --> ManualFix[Manual Fix Required]
```

### Authentication Error Flow (OTP / Login)
```mermaid
flowchart LR
  Login[Start Login] --> SendOTP[Send OTP]
  SendOTP --> |200| WaitForOTP[Wait for OTP]
  SendOTP --> |RateLimit| RateLimited[Rate Limited]
  RateLimited --> Cooldown[Show cooldown message]

  WaitForOTP --> |OTP Valid| CreateSession[Create Session]
  WaitForOTP --> |OTP Invalid| OTPFailed[Show invalid OTP]
  OTPFailed --> RetryOTP{Retry OTP?}
  RetryOTP --> |Yes| SendOTP
  RetryOTP --> |No| ContactSupport

  CreateSession --> MFARequired{MFA Required?}
  MFARequired --> |Yes| MFAFlow[Proceed to MFA]
  MFARequired --> |No| LoginSuccess[Login Success]
```
