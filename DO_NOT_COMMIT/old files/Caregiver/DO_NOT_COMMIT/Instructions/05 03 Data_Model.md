...
## Escrow & Dispute Automation (example triggers/procedures)

The platform uses an escrow ledger to hold guardian payments until job completion and optional dispute resolution. Below is an example Postgres trigger function to auto-schedule escrow release when a job transitions to COMPLETED. Adapt to platform job/payment table names and scheduler mechanism.

```sql
-- Example: Auto-release escrow after 48 hours
...
Dispute Resolution Flow (notes):
- When a dispute is filed, update `payments.status` = 'frozen' and mark related escrow locked.
- Moderators review evidence and either: 1) approve release (set `payments.status` = 'completed' and create payout job), or 2) refund guardian (set `payments.status` = 'refunded').

Caregiver Replacement Flow (notes):
- `assignments` table contains `replaced_by` and `replacement_reason` fields. Workflow:
  1. System or company flags `assignment.status` = 'replaced' and sets `replaced_by` to the new caregiver id.
  2. Keep historical assignment entries for audit; insert new assignment row for replacement with status `assigned` and link to original via `replaced_from_assignment_id` (recommended).
  3. Trigger any necessary re-calculations for payment/proration if replacement impacts delivered hours.
