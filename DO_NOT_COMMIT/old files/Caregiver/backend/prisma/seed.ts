...
async function main() {
  ...
  const providerTx = await prisma.provider_transactions.create({
    data: {
      provider: 'bkash',
      provider_tx_id: `bk_${Date.now()}`,
      amount: 5000.0,
      currency: 'BDT',
      status: 'PENDING',
      payload: { checkout_url: 'https://sandbox.bka.sh/checkout/123' },
    },
  });
  ...
  console.log(`   - 1 Escrow Record with Ledger Entry`);
  console.log(`   - 1 Provider Transaction with Log`);
  console.log('\n✨ All Phase 1 database entities seeded successfully!');
}
