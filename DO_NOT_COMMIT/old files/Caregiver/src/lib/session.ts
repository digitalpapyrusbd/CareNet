...
export async function createSession(options: CreateSessionOptions): Promise<Session> {
  ...
  const now = new Date();
  const expiresIn = options.rememberMe ? EXTENDED_SESSION_TTL : (options.expiresIn || DEFAULT_SESSION_TTL);
  ...
  // Store session in KV
  await kv.setex(`${SESSION_PREFIX}${sessionId}`, expiresIn, JSON.stringify(session));
...
export async function updateSessionActivity(sessionId: string): Promise<void> {
  try {
    ...
    if (ttl > 0) {
      await kv.setex(`${SESSION_PREFIX}${sessionId}`, ttl, JSON.stringify(session));
    }
  } catch (error) {
...
// Extend session TTL
export async function extendSession(sessionId: string, rememberMe: boolean = false): Promise<boolean> {
  try {
    const session = await getSession(sessionId);
    if (!session) return false;
    ...
    await kv.setex(`${SESSION_PREFIX}${sessionId}`, newTTL, JSON.stringify(session));
...
export async function createMFASession(userId: string, userRole: UserRole): Promise<string> {
  const sessionId = uuidv4();
  const now = new Date();
  ...
  await kv.setex(`${SESSION_PREFIX}${sessionId}`, MFA_SESSION_TTL, JSON.stringify(tempSession));
...
export function createSessionCookie(sessionId: string, rememberMe: boolean = false): string {
