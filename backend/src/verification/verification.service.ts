...
import { VerificationStepType, VerificationDecision, UserRole } from '@prisma/client';
...
export class VerificationService {
    ...
    async submitStep(userId: string, submitDto: SubmitVerificationStepDto) {
        ...
        if (existingStep && existingStep.admin_decision === VerificationDecision.ADMIN_APPROVED) {
     ...
     */
    async moderatorReview(stepId: string, moderatorId: string, reviewDto: ModeratorReviewDto) {
        ...
        if (step.admin_decision !== VerificationDecision.PENDING) {
            throw new BadRequestException('This step has already been reviewed by admin');
        }
     ...
     */
    async adminReview(stepId: string, adminId: string, reviewDto: AdminReviewDto) {
        const step = await this.prisma.verification_steps.findUnique({
            where: { id: stepId },
            include: { user: true },
        });
    ...
    private async checkAndMarkVerified(userId: string) {
        ...
        if (approvedSteps === requiredSteps.length) {
            ...
            if (user.role === UserRole.AGENCY_ADMIN || user.role === UserRole.AGENCY_MANAGER) {
