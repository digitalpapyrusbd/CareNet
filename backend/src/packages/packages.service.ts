...
export class PackagesService {
    ...
    async create(companyId: string, createPackageDto: CreatePackageDto) {
        const pkg = await this.prisma.packages.create({
            data: {
                ...
                category: createPackageDto.category as PackageCategory,
    ...
    async findAll(page: number = 1, limit: number = 20, category?: PackageCategory) {
    ...
    async update(id: string, companyId: string, updatePackageDto: UpdatePackageDto) {
        const pkg = await this.prisma.packages.findUnique({
            where: { id },
        });
