...
describe('CompaniesService', () => {
    ...
    describe('create', () => {
        ...
        it('should create a company if user does not have one', async () => {
            ...
            mockPrismaService.companies.create.mockResolvedValue({ id: 'comp-1', ...createDto });
        ...
        it('should throw BadRequestException if user already has a company', async () => {
            mockPrismaService.companies.findUnique.mockResolvedValue({ id: 'existing-comp' });
    ...
    describe('findAll', () => {
        it('should return paginated companies', async () => {
            mockPrismaService.companies.findMany.mockResolvedValue([{ id: 'comp-1' }]);
        ...
        it('should filter by search term', async () => {
            ...
            expect(prisma.companies.findMany).toHaveBeenCalledWith(expect.objectContaining({
    ...
    describe('findOne', () => {
        ...
        it('should throw NotFoundException if company not found', async () => {
            ...
            await expect(service.findOne('comp-1')).rejects.toThrow(NotFoundException);
        });
    });
    ...
    describe('update', () => {
        ...
        it('should update company if user is owner', async () => {
            mockPrismaService.companies.findUnique.mockResolvedValue({ id: 'comp-1', userId: 'user-1' });
            mockPrismaService.companies.update.mockResolvedValue({ id: 'comp-1', ...updateDto });

            const result = await service.update('comp-1', 'user-1', UserRole.AGENCY_ADMIN, updateDto);

            expect(result.company_name).toBe('New Name');
        });

        it('should throw ForbiddenException if user is not owner or admin', async () => {
            mockPrismaService.companies.findUnique.mockResolvedValue({ id: 'comp-1', userId: 'owner' });

            await expect(
                service.update('comp-1', 'user-1', UserRole.AGENCY_ADMIN, updateDto)
        ...
        it('should allow Super Admin to update', async () => {
            ...
            const result = await service.update('comp-1', 'admin', UserRole.SUPER_ADMIN, updateDto);
    ...
    describe('verify', () => {
        it('should verify company', async () => {
            mockPrismaService.companies.findUnique.mockResolvedValue({ id: 'comp-1' });
            mockPrismaService.companies.update.mockResolvedValue({ id: 'comp-1', is_verified: true });

            const result = await service.verify('comp-1', { verification_notes: 'All good' });

            expect(result.is_verified).toBe(true);
            expect(prisma.companies.update).toHaveBeenCalledWith(expect.objectContaining({
