...
export class CompaniesService {
    ...
    async getStatistics(id: string) {
        const [
            ...
            this.prisma.caregivers.count({ where: { company_id: id, deleted_at: null } }),
            this.prisma.caregivers.count({
                where: { company_id: id, is_available: true, deleted_at: null },
            }),
            this.prisma.packages.count({ where: { company_id: id, is_active: true } }),
            this.prisma.jobs.count({ where: { company_id: id } }),
            this.prisma.jobs.count({ where: { company_id: id, status: 'COMPLETED' } }),
        ]);
