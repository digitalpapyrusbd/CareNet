...
describe('CaregiversService', () => {
    ...
    describe('create', () => {
        ...
        it('should create a caregiver profile if one does not exist', async () => {
            ...
            mockPrismaService.caregivers.create.mockResolvedValue({ id: 'cg-1', ...createDto });
        ...
        it('should throw BadRequestException if profile already exists', async () => {
            mockPrismaService.caregivers.findUnique.mockResolvedValue({ id: 'existing' });
    ...
    describe('findAll', () => {
        ...
        it('should filter be enabled conditions', async () => {
            ...
            expect(prisma.caregivers.findMany).toHaveBeenCalledWith(expect.objectContaining({
    ...
    describe('update', () => {
        ...
        it('should update caregiver if user owns profile', async () => {
            mockPrismaService.caregivers.findUnique.mockResolvedValue({ id: 'cg-1', userId });
            mockPrismaService.caregivers.update.mockResolvedValue({ id: 'cg-1', ...updateDto });
        ...
        it('should throw NotFoundException if user does not own profile', async () => {
            mockPrismaService.caregivers.findUnique.mockResolvedValue({ id: 'cg-1', userId: 'other' });

            await expect(service.update('cg-1', userId, updateDto)).rejects.toThrow(NotFoundException);
        });
    });
});
