...
export class JobsService {
    ...
    async create(guardianId: string, createJobDto: CreateJobDto) {
        ...
        const job = await this.prisma.jobs.create({
            data: {
                ...
                end_date: createJobDto.end_date ? new Date(createJobDto.end_date) : new Date(new Date().setMonth(new Date().getMonth() + 1)),
    ...
    async findAll(userId: string, userRole: UserRole, page: number = 1, limit: number = 20) {
        ...
        if (userRole === UserRole.GUARDIAN) {
            ...
            const caregiver = await this.prisma.caregivers.findUnique({ where: { userId } });
            if (caregiver) {
                where.assignments = {
    ...
    async update(id: string, userId: string, updateJobDto: UpdateJobDto) {
        ...
        const updatedJob = await this.prisma.jobs.update({
            where: { id },
            data: {
                start_date: updateJobDto.start_date ? new Date(updateJobDto.start_date) : undefined,
                end_date: updateJobDto.end_date ? new Date(updateJobDto.end_date) : undefined,
    ...
    async assignCaregivers(id: string, userId: string, assignDto: AssignCaregiverDto) {
        const job = await this.prisma.jobs.findUnique({ where: { id } });
    ...
    async acceptJob(id: string, userId: string) {
        ...
        const allAccepted = allAssignments.every((a) => a.status === AssignmentStatus.ACTIVE);
