...
describe('JobsService', () => {
    ...
    describe('create', () => {
        ...
        it('should create a job if package and patient exist', async () => {
            ...
            expect(prisma.jobs.create).toHaveBeenCalledWith(expect.objectContaining({
        ...
        it('should throw NotFoundException if package does not exist', async () => {
            ...
            await expect(service.create(guardianId, createDto)).rejects.toThrow(NotFoundException);
        });

        it('should throw ForbiddenException if patient does not belong to guardian', async () => {
            mockPrismaService.packages.findUnique.mockResolvedValue({ id: 'pkg1', price: 100 });
            mockPrismaService.patients.findUnique.mockResolvedValue({
                id: 'pat1',
                guardian_id: 'other-guardian',
            });

            await expect(service.create(guardianId, createDto)).rejects.toThrow(ForbiddenException);
        });
    });
    ...
    describe('findAll', () => {
        it('should return paginated jobs based on role', async () => {
            ...
            expect(prisma.jobs.findMany).toHaveBeenCalledWith(expect.objectContaining({
    ...
    describe('findOne', () => {
        const job = { id: 'job1', guardian_id: 'guardian1', company_id: 'comp1', assignments: [] };

        it('should return job if user is authorized', async () => {
            mockPrismaService.jobs.findUnique.mockResolvedValue(job);

            const result = await service.findOne('job1', 'guardian1', UserRole.GUARDIAN);
        ...
        it('should throw ForbiddenException if user is not authorized', async () => {
            ...
            await expect(service.findOne('job1', 'other', UserRole.GUARDIAN)).rejects.toThrow(ForbiddenException);
        });
    });

    describe('update', () => {
        it('should update job if pending and user is guardian', async () => {
            const job = { id: 'job1', guardian_id: 'guardian1', status: JobStatus.PENDING_ASSIGNMENT };
            mockPrismaService.jobs.findUnique.mockResolvedValue(job);
            mockPrismaService.jobs.update.mockResolvedValue({ ...job, special_instructions: 'New' });

            await service.update('job1', 'guardian1', { special_instructions: 'New' });

            expect(prisma.jobs.update).toHaveBeenCalled();
        });

        it('should throw BadRequest if job is not pending', async () => {
            const job = { id: 'job1', guardian_id: 'guardian1', status: JobStatus.ACTIVE };
            mockPrismaService.jobs.findUnique.mockResolvedValue(job);

            await expect(service.update('job1', 'guardian1', {})).rejects.toThrow(BadRequestException);
        });
    });
    ...
    describe('assignCaregivers', () => {
        it('should assign caregivers if user is company admin', async () => {
            ...
            await service.assignCaregivers('job1', 'user1', { caregiver_ids: ['cg1'] });
