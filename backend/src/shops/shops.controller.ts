...
import { CreateShopDto, CreateProductDto, CreateOrderDto } from './dto/shop.dto';
...
export class ShopsController {
    ...
    createShop(@CurrentUser('id') userId: string, @Body() createDto: CreateShopDto) {
        return this.shopsService.createShop(userId, createDto);
    }

    @Public()
    @Get()
    findAllShops(@Query() query: any) {
        return this.shopsService.findAllShops(query.page, query.limit);
    }
    ...
    createProduct(@Param('id') shopId: string, @Body() createDto: CreateProductDto) {
    ...
    @Post('orders')
    createOrder(@CurrentUser('id') userId: string, @Body() createDto: CreateOrderDto) {
    ...
    @Roles(UserRole.SHOP_ADMIN, UserRole.SHOP_MANAGER)
    updateOrderStatus(@Param('id') orderId: string, @Body() body: { status: string }) {
