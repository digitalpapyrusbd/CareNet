...
export function UniversalNav({ userRole, showBack = true }: UniversalNavProps) {
  ...
  const getDashboardPath = () => {
    switch (userRole) {
      ...
      case 'agency-manager': return '/agency-manager/dashboard';
      case 'patient': return '/patient/dashboard';
      case 'shop': return '/shop/dashboard';
      case 'shop-manager': return '/shop-manager/dashboard';
  ...
  const getSettingsPath = () => {
    switch (userRole) {
      case 'guardian': return '/guardian/settings';
      case 'caregiver': return '/caregiver/settings';
      case 'agency': return '/agency/settings';
      case 'agency-manager': return '/agency-manager/settings';
      case 'patient': return '/patient/settings';
      case 'shop': return '/shop/settings';
      case 'shop-manager': return '/shop-manager/settings';
      case 'moderator': return '/moderator/settings';
  ...
  const navItems = [
    ...
    },
    {
      ...
      label: 'Assistant',
      action: () => {
        // Open AI assistant modal or navigate to assistant page
  ...
  return (
    <>
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-50 bg-white/90 backdrop-blur border-t border-white/50 safe-area-inset-bottom">
        <div className="grid grid-cols-5 h-16">
          {navItems.map((item) => {
            ...
            const isActive = pathname === getDashboardPath() && item.id === 'home';
            return (
              <button
                key={item.id}
                onClick={item.action}
      ...
      {/* Search Modal */}
      {showSearch && (
        <div className="fixed inset-0 z-50 flex items-start justify-center pt-20 md:pt-32 bg-black/20 backdrop-blur-sm">
      ...
      {/* Profile Menu Modal */}
      {showProfileMenu && (
        <div className="fixed inset-0 z-50 flex items-start justify-end pt-16 md:pt-20 pr-4 bg-black/20 backdrop-blur-sm">
