    ...
    <>
      ...
      <div className="min-h-screen bg-gradient-to-br from-emerald-50 to-green-100 dark:from-gray-900 dark:to-emerald-950 px-4 py-10 pb-24 md:pt-14">
        <div className="max-w-2xl mx-auto finance-card p-8">
          <h1 className="text-2xl font-semibold mb-6" style={{ color: '#535353' }}>{t('page.heading.updatestock')}</h1>
          ...
          <div className="space-y-4">
            <div>
              ...
              <Input
                ...
                placeholder={t('page.placeholder.10or3')}
              />
              <p className="text-xs mt-1" style={{ color: '#848484' }}>New stock: {parseInt(currentStock) + (parseInt(adjustment) || 0)}</p>
            </div>

            <div>
              <label className="text-sm font-medium" style={{ color: '#535353' }}>{t('page.text.notes1')}</label>
              <Textarea value={notes} onChange={(e) => setNotes(e.target.value)} className="mt-2 bg-white/60 border-white/60" rows={3} placeholder={t('page.placeholder.stockadjustmentreaso')} />
            </div>
          </div>
