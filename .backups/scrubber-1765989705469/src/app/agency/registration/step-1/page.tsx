      ...
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 to-indigo-100 dark:from-gray-900 dark:to-blue-950 px-4 py-12 pb-24 md:pt-14">
        <div className="w-full max-w-md finance-card p-8">
          <div className="text-center mb-6">
            ...
            <p className="text-sm mb-2" style={{ color: '#848484' }}>{t('page.text.step1of5')}</p>
            <h1 className="text-2xl font-semibold" style={{ color: '#535353' }}>{t('page.heading.registeryouragency')}</h1>
            <p className="text-sm" style={{ color: '#848484' }}>{t('page.text.phoneverification')}</p>
          </div>

          <div className="space-y-4">
            <div>
              <label className="text-sm font-medium" style={{ color: '#535353' }}>Phone Number *</label>
              <Input value={form.phone} onChange={(e) => update('phone', e.target.value)} className="mt-2 bg-white/60 border-white/60" placeholder={t('page.placeholder.01xxxxxxxxx')} />
            </div>
          ...
          <div className="mt-6 p-4 rounded-2xl" style={{ background: 'rgba(142,197,252,0.1)' }}>
            <p className="text-xs" style={{ color: '#535353' }}>{t('page.text.progress20complete')}</p>
