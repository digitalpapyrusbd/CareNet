      ...
      <div className="min-h-screen bg-gradient-to-br from-violet-50 to-purple-100 dark:from-gray-900 dark:to-violet-950 px-4 py-10 pb-24 md:pt-14">
        <div className="max-w-3xl mx-auto finance-card p-8">
          <div className="flex items-center justify-between mb-6">
            <h1 className="text-2xl font-semibold" style={{ color: '#535353' }}>{t('page.heading.addnewproduct')}</h1>
          ...
          <div className="space-y-4">
            <div>
              <label className="text-sm font-medium" style={{ color: '#535353' }}>{t('page.text.productimages')}</label>
            ...
            <div>
              ...
              <Input value={form.name} onChange={(e) => update('name', e.target.value)} className="mt-2 bg-white/60 border-white/60" placeholder={t('page.placeholder.egdigitalthermometer')} />
            </div>

            <div>
              <label className="text-sm font-medium" style={{ color: '#535353' }}>Description *</label>
              <Textarea value={form.description} onChange={(e) => update('description', e.target.value)} className="mt-2 bg-white/60 border-white/60" rows={4} placeholder={t('page.placeholder.productdetailsandspe')} />
            </div>
            ...
            <div>
              ...
              <select value={form.category} onChange={(e) => update('category', e.target.value)} className="mt-2 w-full rounded-2xl border bg-white/60 px-4 py-3" style={{ borderColor: 'rgba(255,255,255,0.6)', color: '#535353' }}>
                <option value="Medicines">{t('page.text.medicines')}</option>
                <option value="Equipment Sale">{t('page.text.equipmentsale')}</option>
                <option value="Equipment Rental">{t('page.text.equipmentrental')}</option>
                <option value="Services">{t('page.text.services')}</option>
              </select>
            </div>
